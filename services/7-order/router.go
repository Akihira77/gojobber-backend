...
func MainRouter(db *gorm.DB, cld *util.Cloudinary, app *fiber.App) {
	...
	api.Post("/:orderId/refund", nil)
	api.Post("/:orderId/complete", oh.BuyerMarkOrderAsComplete)
	api.Post("/:orderId/cancel", oh.SellerCancellingOrder)
	api.Post("/deadline/request-extension/:orderId", oh.RequestDeadlineExtension)
	api.Post("/deadline/approve-extension/:orderId", oh.ApproveDeadlineExtension)
	api.Post("/deliver/:orderId", nil)
